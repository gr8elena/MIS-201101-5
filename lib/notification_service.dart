...
import 'package:laboratoriska201101/serviceLocation.dart';
...
class NotificationService {
  ...
  Location location = Location("Faculty", 42.004186212873655, 21.409531941596985);
  ...
  Future<void> checkLocationAndNotify() async {
    if (await Permission.locationWhenInUse.serviceStatus.isEnabled) {
      ...
      LocationService().getCurrentLocation().then((value) {
        if ((value.latitude < location.latitudeFaculty + 0.01 &&
            value.latitude > location.latitudeFaculty - 0.01) &&
            (value.longitude < location.longitudeFaculty + 0.01 &&
                value.longitude > location.longitudeFaculty - 0.01)) {
