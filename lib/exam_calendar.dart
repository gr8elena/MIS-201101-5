...
class CalendarWidget extends StatelessWidget {
  ...
  _DataSource _getCalendarDataSource() {
    return _DataSource(exams.map((exam) => _convertToAppointment(exam)).toList());
  }

  Appointment _convertToAppointment(Exam exam) {
    return Appointment(
  ...
  void _handleDateTap(BuildContext context, DateTime selectedDate) {
    List<Exam> selectedExams = exams.where((exam) =>
    exam.timestamp.year == selectedDate.year &&
        exam.timestamp.month == selectedDate.month &&
        exam.timestamp.day == selectedDate.day).toList();
